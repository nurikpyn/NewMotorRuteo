...
public class MultiTypeMatrix implements VehicleRoutingTransportCosts {
    ...
    public double getDistance(Location from, Location to, double v, Vehicle vehicle) {
        if (vehicle==null){
            return this.getDefaultMatrix().getDistances()[from.getIndex()][to.getIndex()];
        }
        return this.matrices.get(vehicle.getType().getProfile()).getDistances()[from.getIndex()][to.getIndex()];
    }
}
